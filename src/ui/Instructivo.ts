...
const FAQ: { id: string; pregunta: string; puntos: string[] }[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      "⏺ ENHEBRAR EL CARRO: coloca la pieza sobre las guías con el gizmo y suéltala. Ahí queda VINCULADA: por cada guía que la atraviesa se le abre un canal REDONDO de verdad en la malla, del diámetro del tubo más la holgura de deslizamiento — como el orificio pasante del carro de una prensa real. Puede ir enhebrada en una guía o en varias. Si la vuelves a mover, los canales se rehacen donde toque; si la sacas de las guías, se los quita. La guía tiene que venir alineada con un eje de la pieza (hasta 12° de desvío): el carro va a escuadra con sus barras, también en la máquina de verdad.",
      "⏺ TOPES Y SAFETY PINS: el TOPE DE GUÍA es el espaciador de goma que se monta sobre la barra — suéltalo cerca de ella y se centra en su recta, alineado. Desde ahí acota el recorrido: el carro se detiene en él en vez de llegar abajo, y puedes poner topes a los dos lados. El SAFETY PIN es el pasador cromado que entra en los pinholes de un pilar, con largo y diámetro ajustables, y hace de brazo de seguridad o de gancho.",
  ...
  },
  {
    ...
    puntos: [
      ...
      "🦶 PISAR UNA SUPERFICIE O PEDAL: el pie no siempre toca el suelo. En una prensa de piernas pisa la plataforma, en una extensión de rodillas queda al aire (cadena abierta) y sentado en un banco alto cuelga. Toca la pierna de la figura y luego la pieza donde apoya: la IK resuelve cadera, rodilla y tobillo, y el pie VIAJA CON LA PIEZA — si el pedal sube, la pierna lo acompaña. Soltar apoyos suelta manos y pies.",
