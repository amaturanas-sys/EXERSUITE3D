...
const FAQ: { id: string; pregunta: string; puntos: string[] }[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      "🦶 PISAR UNA SUPERFICIE O PEDAL: el pie no siempre toca el suelo. En una prensa de piernas pisa la plataforma, en una extensión de rodillas queda al aire (cadena abierta) y sentado en un banco alto cuelga. Toca la pierna de la figura y luego la pieza donde apoya: la IK resuelve cadera, rodilla y tobillo, y el pie VIAJA CON LA PIEZA — si el pedal sube, la pierna lo acompaña. Soltar apoyos suelta manos y pies. La PLACA NO TIENE QUE ESTAR A NIVEL: al pisar se guarda la cara que tocas, así que sobre la plataforma inclinada de una prensa el pie se ACUESTA sobre ella y la planta se posa en su superficie en vez de atravesarla. Si el punto que marcas le queda lejos a la pierna, el pie se apoya en el sitio más cercano de esa misma cara al que llega — una persona pisa donde alcanza.",
