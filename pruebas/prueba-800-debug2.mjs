...
for (const t of TAMANOS) {
  ...
  console.log("RAF/RO check:", JSON.stringify(await page.evaluate(async () => {
    const t0=performance.now(); let frames=0; const stop=t0+600;
    await new Promise((res)=>{const tick=()=>{frames++; if(performance.now()<stop) requestAnimationFrame(tick); else res();}; requestAnimationFrame(tick);});
    let roFired=0; const ro=new ResizeObserver(()=>{roFired++;}); ro.observe(document.querySelector("#simbar"));
    await new Promise(r=>setTimeout(r,600));
    const cs=getComputedStyle(document.documentElement);
    // fuerza un cambio de tamano de la barra
    const sim=document.querySelector("#simbar"); sim.style.paddingTop="20px";
    await new Promise(r=>setTimeout(r,600));
    const despues=cs.getPropertyValue("--simbar-h"); sim.style.paddingTop="";
    await new Promise(r=>setTimeout(r,600));
    return {frames, roFired, simbarH_tras_forzar:despues, simbarH_final:cs.getPropertyValue("--simbar-h"), toolbarH:cs.getPropertyValue("--toolbar-h"), simH:sim.offsetHeight};
  })));
